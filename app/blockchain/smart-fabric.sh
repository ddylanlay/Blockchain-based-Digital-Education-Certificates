#!/bin/bash
# smart-fabric.sh - Intelligent Fabric network management
...
function checkNetworkHealth() {
    ...
    if ! timeout 10 docker exec cli peer version > /dev/null 2>&1; then
        echo "❌Peer CLI not responding. Network restart recommended."
        return 1
    fi
    ...
    if ! timeout 10 peer channel list > /dev/null 2>&1; then
        echo "❌ Cannot query channels. Network restart recommended."
        return 1
    fi
...
function startOrResume() {
    ...
    if docker ps | grep -q "peer0.org1.example.com"; then
        ...
        if checkNetworkHealth; then
            echo "✅ Network is healthy. Resuming work..."
            showNetworkStatus
            return 0
        else
            echo "⚠️  Network has issues. Attempting restart..."
            ./network.sh down
        fi
    fi
...
function showNetworkStatus() {
    ...
    echo "💡 Your assets will persist until you run './network.sh down'"
}
...
case $COMMAND in
    ...
    "down")
        cd fabric-samples/test-network
        ./network.sh down
        echo "🛑 Network stopped and data cleared"
        ;;
    *)
        ...
        echo ""
        echo "🎯 Recommended daily workflow:"
